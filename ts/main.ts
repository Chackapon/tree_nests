...
import {
    ...
    ItemStack,
    BlockPermutation,
    LocationInUnloadedChunkError, Block, VectorXZ, Vector3, Dimension,
...
import {
    ...
    EMPTY_STATE_NAME, INHABITANT_MOB_TYPES,
} from "./default_const.js";
...
world.afterEvents.worldLoad.subscribe((event) => {
    ...
    setDebugMode( loadWorldData( `${MOD_NAMESPACE}:debug_mode`, HOLLOW_CHANCE ) as boolean );
    setHollowChance( loadWorldData( `${MOD_NAMESPACE}:hollow_chance`, HOLLOW_CHANCE ) as number );
    setInhabitantChance( loadWorldData( `${MOD_NAMESPACE}:inhabitant_chance`, INHABITANT_CHANCE ) as number );
});
...
world.afterEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    if ( block.permutation.getState(
        `${MOD_NAMESPACE}:${EMPTY_STATE_NAME}` as any
        ) === true
    ) return;
    ...
    if ( block.permutation.getState(
        `${MOD_NAMESPACE}:${INHABITANT_STATE_NAME}` as any
        ) === "empty" ) {
        ...
        block.setPermutation( block.permutation.withState(`${MOD_NAMESPACE}:${INHABITANT_STATE_NAME}` as any, "empty") );
    }


    block.setPermutation( block.permutation.withState(`${MOD_NAMESPACE}:${EMPTY_STATE_NAME}` as any, true) ); //todo: maybe different texture? also more states, maybe enchanted, bird inside, etc

    // Apply damage to brush item
    const container = player.getComponent("minecraft:inventory")?.container;
    const slot = player.selectedSlotIndex;
    const brush_item = container?.getItem( slot );
    if (!brush_item) return;
 ...
 */
function detectTrunk( top_block: Block ) {
 ...
 * @returns {boolean}
 */
function randomWorldChance( location: VectorXZ, chance: number ) {
 ...
 * @param location
 */
export function randomCardinalDirection( location: Vector3 ) {
 ...
 * @returns {*}
 */
function placeNest( dimension: Dimension, trunk_coords: Vector3 ) {
    ...
    const key = rand_dir as keyof object
    const offset: Vector3 = direction_vector[key];
    ...
    if ( rolled_chance < INHABITANT_CHANCE ) {
        ...
        const permutation = nest_block.permutation.withState( `${MOD_NAMESPACE}:${INHABITANT_STATE_NAME}` as any, "mystery" );
        nest_block.setPermutation( permutation );
    }
 ...
 * @param chunkZ
 */
function processChunk(dimension: Dimension, chunkX: number, chunkZ: number) {
            ...
            for ( let z = -2; z < 3; z++ ) {
                ...
                if (!processed_chunks.has( key )) {
                    ...
                    try {
                        ...
                        saveWorldData( `${MOD_NAMESPACE}:chunks`, processed_chunks );
                    }
                    catch (error) {
