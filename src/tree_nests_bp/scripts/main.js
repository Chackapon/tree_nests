// TREENESTS ADDON
import { world, system, ItemStack, BlockPermutation, LocationInUnloadedChunkError, } from "@minecraft/server";
import { log, setDebugMode, } from "./logger.js";
...
world.afterEvents.playerInteractWithBlock.subscribe((event) => {
    ...
    if (block.permutation.getState(`${MOD_NAMESPACE}:${EMPTY_STATE_NAME}`) === true)
        return;
    ...
    for (let x = startX; x < startX + 16; x++) {
        for (let z = startZ; z < startZ + 16; z++) {
            ...
            if (top_block.typeId.endsWith("_log") && top_block.permutation.getState("pillar_axis") === "y") {
                ...
                if (rolled_chance >= HOLLOW_CHANCE)
                    continue;
